...
import { getLocalTransactionById } from '@/lib/api';
...
export default function LocalTransactionDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    (async () => {
      try { const res = await getLocalTransactionById(id); setTx(res.data.transaction ?? res.data); }
  ...
  return (
    <div className="min-h-screen bg-bg-light mx-auto max-w-2xl px-4 sm:px-6 lg:px-10 pt-8 lg:pt-10">
      <div className="mb-6 flex items-center justify-between">
        ...
        <h1 className="font-cairo text-xl font-bold text-text-primary">تفاصيل المعاملة المحلية</h1>
        <div className="w-6" />
      </div>
      <div className="mb-6 text-center">
        <p className={`font-manrope text-4xl font-bold ${pos ? 'text-success' : 'text-error'}`}>
          {pos ? '+' : '-'}{parseFloat(tx.amount).toLocaleString('en')} ج.م
        </p>
      </div>
