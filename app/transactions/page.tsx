...
interface Transaction {
  ...
  category_name?: string; created_at: string; is_ai?: boolean;
}

const getTypeEmoji = (type: string) => {
  switch (type) { case 'deposit': return '💰'; case 'withdraw': return '💸'; case 'transfer': return '📤'; case 'manual_expense': return '🧾'; case 'manual_income': return '💵'; case 'ai_expense': case 'ai_income': return '🤖'; default: return '💳'; }
};
const getTypeLabel = (type: string) => {
  switch (type) { case 'deposit': return 'إيداع'; case 'withdraw': return 'سحب'; case 'transfer': return 'تحويل'; case 'manual_expense': return 'مصروف يدوي'; case 'manual_income': return 'دخل يدوي'; case 'ai_expense': return 'مصروف AI'; case 'ai_income': return 'دخل AI'; default: return type; }
};
...
export default function TransactionHistoryPage() {
  ...
  const fetchTransactions = useCallback(async (pg = 1, append = false) => {
    try {
      const res = await getTransactions(pg, 20, filter || undefined);
  ...
  return (
    <div className="min-h-screen bg-bg-light mx-auto max-w-4xl px-4 sm:px-6 lg:px-10 pt-8 lg:pt-10">
      <div className="mb-3 flex items-center justify-between">
        ...
        <h1 className="font-cairo text-xl font-bold text-text-primary">سجل المعاملات</h1>
        <div className="w-6" />
      </div>
