...
import { depositMain } from '@/lib/api';
...
export default function DepositPage() {
  ...
  const handleDeposit = async () => {
    ...
    try {
      const res = await depositMain(num, uuidv4());
      const params = new URLSearchParams({
        amount: String(num),
        newBalance: String(res.data.transaction?.new_balance ?? res.data.newBalance ?? ''),
        transactionId: String(res.data.transaction?.id ?? res.data.transactionId ?? ''),
      });
  ...
  return (
    <div className="min-h-screen overflow-y-auto bg-bg-light mx-auto max-w-2xl px-4 sm:px-6 lg:px-10 pt-8 lg:pt-10 pb-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <h1 className="font-cairo text-xl font-bold text-text-primary">إيداع</h1>
        <div className="w-6" />
      </div>
      ...
      <div className="mb-5 rounded-2xl bg-primary-dark p-6 text-center shadow-md">
        ...
        <p className="font-manrope text-2xl font-bold text-gold">
      ...
      <div className="mb-5 flex items-center gap-2 rounded-xl bg-primary-surface p-3">
        ...
        <p className="flex-1 font-cairo text-xs text-primary">المبلغ هيتضاف لرصيدك فوراً بعد التأكيد</p>
      </div>
